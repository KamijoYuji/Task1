...
vector<double> vecAddScaled(const vector<double>& x, double t, const vector<double>& d) {
    vector<double> y = x;
    for (size_t i = 0; i < y.size(); ++i) y[i] += t * d[i];
    return y;
}

// ---- Минимизируемая функция и её обёртка для одномерного поиска ----
...
// ---- Одномерная минимизация (Свенн + золотое сечение) ----
// (скопированы из первого файла, без изменений)
...
vector<double> steepest_descent(double(*f)(vector<double>, double, vector<double>),
                                vector<double> x0,
    ...
    while (true) {
        ...
        double t_k = gold(f, x, d, interval, tol, false);

        // Шаг 7
        vector<double> x_new = vecAddScaled(x, t_k, d); // x + t_k * (-grad)

        // Шаг 8
        double dx = norm(vecAddScaled(x_new, -1.0, x));
        double df = fabs(f_value(x_new) - f_value(x));
        bool current_ok = (dx < eps2) && (df < eps2);
...
int main() {
    ...
    vector<double> result = steepest_descent(f_wrap, x0, eps1, eps2, M);
